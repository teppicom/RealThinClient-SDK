...
{ This is a Multi-Threaded Web Socket Client Test Project.

  It posts a HTTP request to a Web Server specified in the "SrvAddr" edit field
  on port specified in the "SrvPort" edit field and sends a request to establish
  a Web Socket connection using URI specified in the "SrvURI" edit field.

  This example Web Socket Client is implemented to communicate with the
  "WSockSrvTest" Project from the "SDKExamples_QuickStart" Project Group,
  but the same principles apply to any Web Socket Server, with the only
  difference that diffeent Servers will probably be using different URIs,
  might require more information in Request headers to establish a Web
  Socket connection and will be sending and expecting to receive some
  specific data in Web Socket Frames and not some random content.
...
uses
  ...
  Dialogs, rtcDataCli, rtcInfo, rtcConn, rtcHttpCli, StdCtrls,
  rtcThrPool, ExtCtrls, rtcLog, rtcSystem, rtcThrJobs;
...
type
  TForm1 = class(TForm)
    ...
    btnSendTxtChunks: TButton;
    btnSendExeChunks: TButton;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    cbLOG: TComboBox;
...
procedure TForm1.btnConnectClick(Sender: TObject);
  ...
  if not Client.isConnecting then // not already connected or connecting ...
    ...
    Client.ServerAddr:=SrvAddr.Text;
    Client.ServerPort:=SrvPort.Text;
    ...
    SockReq.Request.URI:=SrvURI.Text;
