...
4.) Make the RTC SDK accessible from XCode - for iOS development with Delphi XE2
...
You can add the path to the RTC SDK 鏑ib・folder (located on your Windows PC, made accessible to Mac over LAN) 
to the FPC search path. Unfortunatelly, there  is no 菟arameter・for adding FPC search paths in XCode directly, 
so you will need to do this manually for every XCode Project. And not only once, but every time you recreate 
XCode Project files by using the 電pr2xcode・tool, because all your changes will be overwritten by "dpr2xcode". 
...
The path to the RTC SDK 鏑ib・folder needs to be added as two new ・Fu・parameters. Once for iOS-Simulator and 
once for iOS-Device compilation, both of are configured through the 都hellScript・parameter.
...
through a network share named 迭TC_SDK・(read/write access rights to that folder will be required for FPC 
